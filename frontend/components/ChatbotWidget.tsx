...
} from '@/components/smoothui/ai-core';
...
export default function ChatbotWidget({
  ...
  const handleOptionClick = (opt: string, messageId: number) => {
    ...
    setMessages((prev) =>
      prev.map((m) => (m.id === messageId ? { ...m, options: undefined } : m)),
    );
  ...
  return (
    ...
    >
        ...
        {isChatOpen && (
          ...
          >
                      ...
                      isAssistant ? (
                        <div className="w-6 h-6 rounded-full overflow-hidden flex items-center justify-center bg-white border border-neutral-200/90 shadow-2xs">
                          <AIOrbFace
                            ...
                            amplitude={
                              isLastBotMessage ? simulatedAmplitude : 0
                            }
                            gaze={false}
            ...
            <form
              ...
              className="p-3 border-t border-neutral-200 bg-transparent flex gap-2"
            >
              <input
                type="text"
                value={inputVal}
                ...
                onBlur={() => {
                  if (!isTyping && aiState === 'listening' && !inputVal.trim())
                    setAiState('idle');
                }}
      ...
      <button
        ...
        aria-label={
          isChatOpen
            ? 'Close Virtual Arijit assistant'
            : 'Chat with Virtual Arijit AI Assistant'
        }
        className={cn(
