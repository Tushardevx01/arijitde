...
export default function Onboarding() {
  ...
  const [clientStep, setClientStep] = useState<
    'EMAIL' | 'OTP_VERIFY' | 'ACCOUNT_SELECT' | 'PAN_VERIFY'
  >('EMAIL');
  ...
  const initializeGoogleSignIn = () => {
    ...
    if (typeof window !== 'undefined' && (window as any).google) {
      try {
        ...
        if (newUserBtn) {
          (window as any).google.accounts.id.renderButton(newUserBtn, {
            theme: 'dark',
            size: 'large',
            width: '100%',
            text: 'signup_with',
        ...
        const clientBtn = document.getElementById(
          'google-signin-button-client',
        );
        if (clientBtn) {
          (window as any).google.accounts.id.renderButton(clientBtn, {
            theme: 'dark',
            size: 'large',
            width: '100%',
            text: 'signin_with',
  ...
  const handleRegisterSubmit = async (e: React.FormEvent) => {
    ...
    if (isAdminLogin && adminMode === 'PASSWORD') {
      ...
      try {
        ...
        setError(
          'Unable to connect to authentication server. Is the backend running?',
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleClientPanVerify = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setError(
          data.error ||
            'PAN verification failed. Please check the number and try again.',
        );
      }
    } catch (err) {
      setError('Error connecting to server. Please try again.');
      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center items-center px-6 pt-36 pb-24 max-w-5xl mx-auto w-full">
        ...
        {flow === 'SELECT' && (
          ...
          >
            ...
            <p className="text-muted-foreground text-sm md:text-base max-w-lg mb-12">
              Choose your profile below to enter your customized AMFI-registered
              Mutual Fund distribution dashboard.
            </p>
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground text-xs leading-relaxed font-sans mt-2 pr-4">
                      Registered portfolios under Arindam De or Arijit De. Log
              ...
              >
                <div className="flex flex-col gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-muted-foreground text-xs leading-relaxed font-sans mt-2 pr-4">
                      Create an account to start analyzing, optimizing, and
                      tracking your assets under management.
                    </p>
                  </div>
                </div>
                <div className="mt-8 text-xs font-mono text-primary opacity-60 group-hover:opacity-100 flex items-center gap-1.5 transition-all">
              ...
              {isAdminLogin && (
                <div className="text-right">
                  <button
                    ...
                    onClick={() => {
                      setAdminMode(
                        adminMode === 'PASSWORD' ? 'OTP' : 'PASSWORD',
                      );
                      setPassword('');
                      setError(null);
                    }}
            ...
            {!isAdminLogin && (
              <>
                <div className="relative my-6 flex items-center justify-center">
                  ...
                  <span className="relative z-10 px-3 bg-white/50 backdrop-blur-md text-[10px] font-mono text-muted-foreground uppercase tracking-widest rounded-full">
                    Or Continue With
                  </span>
                </div>
        ...
        {flow === 'OTP_VERIFY' && (
          <div className="w-full max-w-md bg-white/30 border border-white/20 rounded-3xl p-8 shadow-2xl backdrop-blur-xl relative text-center">
            ...
            >
              <div>
                <label className="block text-[11px] font-mono uppercase tracking-wider text-muted-foreground mb-1.5 text-center">
                  6-Digit OTP Code
                </label>
                <input
                  type="text"
          ...
          <div className="w-full max-w-md bg-white/30 border border-white/20 rounded-3xl p-8 shadow-2xl backdrop-blur-xl relative text-left">
            ...
            <p className="text-muted-foreground text-xs font-sans mb-6">
              ...
              {clientStep === 'OTP_VERIFY' &&
                `Enter the 6-digit code sent to ${clientEmail}`}
              {clientStep === 'ACCOUNT_SELECT' &&
                `Multiple portfolios found under ${clientEmail}. Select yours to proceed:`}
              {clientStep === 'PAN_VERIFY' &&
                `Enter the PAN card number associated with ${selectedAccount?.name || 'your profile'}`}
            </p>
            {/* Error Message */}
            {error && (
            ...
            {clientStep === 'EMAIL' && (
              <>
                ...
                <div className="w-full flex flex-col items-center justify-center">
                  <div
                    id="google-signin-button-client"
                    className="w-full min-h-[40px] flex justify-center"
                  />
                </div>
              </>
              ...
              <form onSubmit={handleClientVerifyOtp} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    value={clientOtp}
                    onChange={(e) =>
                      setClientOtp(e.target.value.replace(/\D/g, ''))
                    }
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      value={enteredPan}
                      onChange={(e) =>
                        setEnteredPan(e.target.value.toUpperCase())
                      }
